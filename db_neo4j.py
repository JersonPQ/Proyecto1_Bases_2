...
class Neo4jDataBase:
    ...
    def __init__(self, user: str ="db_user", 
        ...
        self.uri = "neo4j://172.19.0.1:" + str(port)
        self.authUser = user
    ...
    def insertSurvey(self, survey: dict, idSurvey) -> None:
        with GraphDatabase.driver(uri=self.uri, auth= (self.authUser, self.authPassword)) as driver:
            driver.verify_connectivity()
            driver.execute_query("CREATE (survey : Surver {idSurvey:$idSurvey, title:$title})", {"idSurvey":idSurvey, "title":survey['titulo']})
        driver.close()
    ...
    def insertManyQuestions(self, questions:list, idSurvey) -> list:
        for question in questions:
            with GraphDatabase.driver(uri=self.uri, auth=(self.authUser, self.authPassword)) as driver:
                ...
                driver.execute_query("MATCH (survey : Survey {idSurvey:$idSurvey}) MATCH (question : Question {idPregunta:$idPregunta}) CREATE (survey)-[:PREGUNTA]->(quetion)", {"idSurvey" : idSurvey, "idPregunta" : question['id_pregunta']})
        driver.close()
        return questions
    ...
    def insertQuestion(self, question: dict, idSurvey) -> dict:
        with GraphDatabase.driver(uri=self.uri, auth=(self.authUser, self.authPassword)) as driver:
            driver.verify_connectivity()
            driver.execute_query("CREATE (question : Question {idPregunta:$idPregunta, pregunta:$pregunta})", {"idPregunta" : question['id_pregunta'], "pregunta" : question['pregunta']})
            driver.execute_query("MATCH (survey : Survey {idSurvey:$idSurver}) MATCH (question : Question {idPregunta:$idPregunta}) CREATE (survey)-[:PREGUNTA]->(question)", {"idSurvey" : idSurvey, "idPregunta" : question['id_pregunta']})
        driver.close()
        return question
